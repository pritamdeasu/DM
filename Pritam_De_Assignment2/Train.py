...
##concatenating the features:
    meal_feature_matrix = pd.concat([tmax_tm,cgm_diff,v_max,t_vmax,v2_max,t_v2max],axis=1,ignore_index=True)
...
#    meal_feature_matrix = pd.concat([meal_feature_matrix,dff1,dff2],axis=1)  
    ...
    meal_feature_matrix.columns = (range(0,14))
...
#    meal_feature_matrix = meal_feature_matrix.sample(n=275)
    ...
    ##concatenating the features:
    no_meal_feature_matrix = pd.concat([tmax_tm,cgm_diff,v_max,t_vmax,v2_max,t_v2max],axis=1,ignore_index=True)
...
#    no_meal_feature_matrix = pd.concat([no_meal_feature_matrix,dff1,dff2],axis=1)  
    ...
    no_meal_feature_matrix.columns = (range(0,14))
...
file_path_insulin=input('Please enter full path of Patient 1 inslin data file: ')
file_path_cgm=input('Please enter full path of Patient 1 CGM data file: ')
X_pt1,Y_pt1 =  meal_NoMeal_Data_Extract(file_path_insulin,file_path_cgm)   

file_path_insulin=input('Please enter full path of Patient 2 inslin data file: ')
file_path_cgm=input('Please enter full path of Patient 2 CGM data file: ')
...
clf = MLPClassifier(alpha=1, max_iter=100,random_state=1)
clf.fit(X_pt1,Y_pt1)
...
print(precision_recall_fscore_support(Y_pt2, Y_pred, average='binary',pos_label=1))
...
gpc = GaussianProcessClassifier(kernel=kernel).fit(X_pt1,Y_pt1)
Y_pred = gpc.predict(X_pt2)
acc_score=accuracy_score(Y_pt2,Y_pred)
print("Accuracy of GaussianProcessClassifier 1**2 * RBF(length_scale=1)",acc_score)
...
neigh.fit(X_pt1,Y_pt1)

Y_pred = neigh.predict(X_pt2)
acc_score=accuracy_score(Y_pt2,Y_pred)

print("Accuracy of KNeighborsClassifier n_neighbors=5",acc_score*100)
precision_recall_fscore_support(Y_pt2, Y_pred, average='binary',pos_label=0)
precision_recall_fscore_support(Y_pt2, Y_pred, average='binary',pos_label=1)
...
model=dt.fit(X_pt1,Y_pt1)
Y_pred = model.predict(X_pt2)
acc_score=accuracy_score(Y_pt2,Y_pred)
print("DecisionTreeClassifier: ",acc_score*100)
precision_recall_fscore_support(Y_pt2, Y_pred, average='binary',pos_label=1)
precision_recall_fscore_support(Y_pt2, Y_pred, average='binary',pos_label=0)
